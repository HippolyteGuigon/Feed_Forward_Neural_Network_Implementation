...
class Test(unittest.TestCase):
    ...
    def test__forward_batch(self) -> None:
        ...
        network=neural_network(input_data=data,
                    targets=targets)
        layer_1=layer(64, 784, first_layer=True)
        ...
        self.assertTrue(np.all(np.array([np.round(torch.sum(a).item(),5) for a in prob])==1))
